...
export const config = {
  ...
  databasePath: Boolean(process.env.VERCEL) ? '/tmp/reseller_store.db' : path.resolve(process.cwd(), './data/reseller_store.db'),
  uploadDir: Boolean(process.env.VERCEL) ? '/tmp/proofs' : path.resolve(process.cwd(), './uploads/proofs'),
  blob: {
